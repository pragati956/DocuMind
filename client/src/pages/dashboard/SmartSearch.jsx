...
function Highlight({ text, query }) {
  ...
  return (
    <>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark key={i} className="bg-blue-500/25 text-blue-200 rounded px-0.5">
            {part}
          </mark>
        ) : (
...
/* ─── Search Bar ─── */
function SearchBar({ query, setQuery, onSearch, focused, setFocused, loading }) {
...
/* ─── Recent Searches ─── */
function RecentSearches({ searches, onSearch, onRemove, onClear }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onClear} className="text-gray-700 hover:text-gray-400 text-[10px]">
          Clear all
        </button>
      </div>
...
/* ─── Suggestion Cards ─── */
function SuggestionCards({ onSearch, categories }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {categories.map((s, i) => (
          ...
          >
            <div
              className={`w-10 h-10 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center text-white text-base`}
            >
              {s.icon}
            </div>
            <div>
...
function TrendingSearches({ trending, onSearch }) {
  if (trending.length === 0) return null;
  return (
    <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
      <h3 className="text-white text-sm font-semibold mb-3">Trending Searches</h3>
      <div className="flex flex-wrap gap-2">
        {trending.map(([query, count]) => (
          <button
            key={query}
            onClick={() => onSearch(query)}
            className="px-3 py-1 rounded-full bg-blue-500/10 text-blue-300 text-xs"
          >
            {query} ({count})
          </button>
        ))}
...
function SearchAnalytics({ totalSearches, mostUsedSearch, lastSearch }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      <StatCard title="Total Searches" value={totalSearches} />
      <StatCard title="Most Used" value={mostUsedSearch} />
      <StatCard title="Last Search" value={lastSearch} />
    </div>
  );
}

/* ─── Result Card ─── */
function ResultCard({ result, index, query, onStarToggle, onView, onSummarize }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        boxShadow: hovered
          ? `0 0 32px ${result.dim}, 0 4px 20px rgba(0,0,0,0.25)`
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: 0.3 }}
        className="absolute -top-8 -right-8 w-28 h-28 rounded-full blur-[50px] pointer-events-none"
        style={{ background: result.dim }}
      />
      ...
      <div className="relative z-10 p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p
                className={`text-sm font-semibold truncate transition-colors ${
                  hovered ? "text-white" : "text-gray-100"
                }`}
              >
                {result.title}
              </p>
              <div className="flex items-center gap-2 mt-0.5">
                {result.hasSummary && (
                  <span className="px-2 py-0.5 rounded-full text-[9px] bg-purple-500/15 text-purple-300">
                    AI Summary
                  </span>
                )}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="hidden sm:flex flex-col items-end">
              <span className={`text-xs font-bold ${result.accentText}`}>
                {result.relevance}%
              </span>
            ...
            >
              <FiStar
                className={`text-xs ${starred ? "text-amber-400" : "text-gray-600"}`}
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            {(result.tags || []).map((tag, i) => (
              <span
                key={i}
                className={`px-2 py-0.5 rounded-full text-[9px] font-semibold border ${result.accentText}`}
        ...
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className="overflow-hidden block sm:block"
        >
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.05]">
            <div className="flex items-center gap-2">
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                onClick={() => onSummarize(result.id)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold border border-white/[0.07] bg-white/5 text-gray-300 hover:bg-white/10 transition-all"
              >
...
/* ─── Results Header ─── */
function ResultsHeader({ query, count, activeFilter, setActiveFilter, sortBy, setSortBy }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
      <div>
        ...
        <p className="text-gray-600 text-[11px] mt-0.5">
          Semantic AI search · sorted by relevance
        </p>
      </div>
      <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5" style={{ scrollbarWidth: "none" }}>
        ...
        <select
          ...
          className="bg-[#111827] border border-[#1F2937] text-gray-300 text-xs rounded-lg px-2 py-1"
        >
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-sm mb-6 max-w-xs">
        Try different keywords, check spelling, or search using natural language.
      </p>
...
export default function SmartSearch() {
  ...
  useEffect(() => {
    ...
    recents.forEach((item) => {
      countMap[item.query] = (countMap[item.query] || 0) + 1;
    });
  ...
  const handleSearch = async (q) => {
    ...
    try {
      ...
      setRecents(historyData.history);
      setFocused(false);
    } catch (error) {
      console.error(error);
  ...
  const formattedResults = results.map((doc) => {
    // Determine the correct extension fallback
    const ext = doc.fileType?.includes("pdf") ? ".pdf" 
              : doc.fileType?.includes("word") ? ".docx" 
              : doc.fileType?.includes("image") ? ".png" 
              : ".txt";

    return {
      id: doc._id,
      title: doc.title,
      // Force the extension onto the name so the modal routes it properly
      name: doc.title.includes(".") ? doc.title : `${doc.title}${ext}`,
      file: doc.title,
      fileUrl: doc.fileUrl,
      // include original MIME/file type so the preview modal can choose an inline viewer
      ...
      type: doc.fileType?.includes("pdf")
        ...
        ? "IMAGE"
        : "TXT",
      excerpt: doc.summary || "No summary available",
      tags: doc.tags || [],
  ...
  const handleView = (id) => {
    const doc = formattedResults.find((d) => d.id === id);
    if (doc) {
      setSelectedDoc(doc);
    }
  };
  ...
  const handleSummarize = async (id) => {
    try {
      ...
      await handleSearch(searchedQuery);
  ...
  const filteredResults = formattedResults.filter((r) => {
  ...
  const mostUsedSearch = trending.length > 0 ? trending[0][0] : "-";
  const lastSearch = recents.length > 0 ? recents[0].query : "-";
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19]" style={{ fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-12">
        ...
        >
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/[0.06] bg-white/[0.03]">
            ...
            <span className="text-gray-500 text-xs">
              {stats?.totalDocs || 0} documents indexed
            </span>
          </div>
        </motion.div>
        ...
        {stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
            <StatCard title="Documents" value={stats.totalDocs} />
            <StatCard title="PDF" value={stats.pdfs} />
            <StatCard title="DOCX" value={stats.docx} />
            <StatCard title="TXT" value={stats.txt} />
            <StatCard title="Summaries" value={stats.summaries} />
          </div>
        )}
      ...
      {selectedDoc && (
        <DocumentPreviewModal document={selectedDoc} onClose={() => setSelectedDoc(null)} />
      )}
    </div>
  );
}