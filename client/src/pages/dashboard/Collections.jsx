...
function ViewCollectionModal({ collectionId, onClose, onDocClick }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-3xl max-h-[90vh] flex flex-col bg-[#111827] rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
...
">
            <motion.div animate={{ borderColor: searchFocused ? "rgba(59,130,246,0.4)" : "rgba(31,41,55,1)" }}
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                ...
                placeholder="Search collections…" className="bg-transparent text-white text-xs placeholder-gray-700 outline-none w-24
sm:w-32
md:w-40" />
              {/* Added Clear Button */}
              <AnimatePresence>
