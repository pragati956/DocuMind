...
<div
 ...
 "
>                {!n.isRead && (
                    <button onClick={() => onMarkRead(n._id)} className="text-xs
px-3
py-1.5
rounded-lg bg-white/5 text-gray-200">Mark read</button>
                )}
                <button onClick={() => onDelete(n._id)} className="text-xs px-3 py-1.5 rounded-lg bg-white/5 text-red-400">Delete</button>
            </div>
        </div>
    );
}
