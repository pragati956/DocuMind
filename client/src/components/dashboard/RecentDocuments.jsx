...
import {
  ...
  FiShare2, FiTrash2, FiEye, FiFilter,
...
function ContextMenu({
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-full mt-1.5 w-48 rounded-2xl border border-[#1F2937] overflow-hidden shadow-2xl z-50"
...
function DocCard({ doc, index, onSummarize, onDelete, onToggleStar }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 pt-3 text-left relative z-10">
...
function EmptyState({ filter }) {
  const msg = filter === "All" ? "No documents found" : `No ${filter} documents found`;
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-16 px-4 col-span-full">
      ...
      <p className="text-white font-semibold text-sm mb-1">{msg}</p>
      <p className="text-gray-600 text-xs mb-5 text-center">
        {filter === "All" ? "Upload a new document to get started." : "Try adjusting your filters."}
      </p>
    </motion.div>
  );
}
...
export default function RecentDocuments() {
  ...
  const filters = [
    "All",
    ...new Set(docs.map(doc => doc.ext.toUpperCase())),
    "Starred"
  ];
    ...
    try {
      ...
      if (res.data.success) {
        const formattedDocs = res.data.documents.map(d => {
          ...
          if(ext==="pptx") accent="#f59e0b";
          if(ext==="xlsx") accent="#10b981";

          let accentData;
          if(ext==="pdf") { accentData={ grad:"from-red-500 to-rose-600", text:"text-red-300" }; }
          else if(ext==="docx") { accentData={ grad:"from-blue-500 to-indigo-600", text:"text-blue-300" }; }
          else if(ext==="pptx") { accentData={ grad:"from-orange-500 to-amber-600", text:"text-orange-300" }; }
          else if(ext==="xlsx") { accentData={ grad:"from-emerald-500 to-green-600", text:"text-emerald-300" }; }
          else { accentData={ grad:"from-purple-500 to-pink-600", text:"text-purple-300" }; }
          ...
          return {
            ...
            starred: d.starred,
            owner: d.uploadedBy?.name || "Unknown",
  ...
  const filtered = docs.filter((d) => {
    ...
    if (activeFilter === "Starred") return d.starred;
    return d.ext.toLowerCase() === activeFilter.toLowerCase();
  });

  const latest = filtered
    .slice()
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 3); // Showing 3 latest on Dashboard
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'Poppins', sans-serif" }} className="mb-6">
        ...
        {/* Removed overflow-hidden from here to fix the cut-off menu issue */}
        <div className="rounded-2xl border border-[#1F2937] bg-[#111827] relative z-10">
          {view === "list" && (
            <>
