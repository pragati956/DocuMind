  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500/20 to-indigo-600/20 border border-blue-500/20 flex items-center justify-center"
      >
        <motion.div
          animate={{ rotate: dragging ? [0, -15, 15, -10, 10, 0] : 0 }}
      ...
      <div className="text-center relative z-10">
        <motion.p
          animate={{ color: dragging ? "#93c5fd" : "#e5e7eb" }}
          className="font-semibold text-sm mb-1"
        >
          {dragging ? "Drop files to upload" : "Drop files here or click to browse"}
        </motion.p>
        <p className="text-gray-600 text-xs">PDF, DOCX, TXT, PNG, JPG — up to 50 MB each</p>
      </div>

      {/* Format badges */}
      {!dragging && (
        <div className="flex items-center gap-1.5 flex-wrap justify-center">
          {["PDF", "DOCX", "TXT", "PNG", "JPG"].map((fmt, i) => (
            <motion.span
              key={i}
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: i * 0.05 }}
              className="px-2 py-0.5 rounded-full bg-white/5 border border-white/[0.08] text-gray-500 text-[10px] font-semibold"
            >
              {fmt}
            </motion.span>
          ))}
...
export default function UploadModal({ onClose }) {
  ...
  const startUpload = () => {
    ...
    files.forEach((file, i) => {
      setUploadStates((s) => ({ ...s, [file.name + file.size]: { progress: 0, status: "uploading" } }));
      ...
      const tick = () => {
        ...
        setUploadStates((s) => ({ ...s, [file.name + file.size]: { progress, status: progress < 100 ? "uploading" : "done" } }));
        if (progress < 100) requestAnimationFrame(tick);
        else if (i === files.length - 1) setTimeout(() => setPhase("done"), 400);
      };
      requestAnimationFrame(tick);
    });
  };

  const allDone = files.length > 0 && files.every((f) => uploadStates[f.name + f.size]?.status === "done");
            ...
            {phase === "done" ? (
              ...
              <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {phase === "idle" && (
                          <button
                            onClick={() => setFiles([])}
                            className="text-gray-600 hover:text-red-400 text-[10px] transition-colors flex items-center gap-1"
                          >
                        ...
                        <AnimatePresence>
                          {files.map((file, i) => (
                            <FileRow
                              ...
                              index={i}
                              onRemove={removeFile}
                              uploadState={uploadStates[file.name + file.size]}
                            />
                          ))}
                        </AnimatePresence>
                      </div>
                      ...
                      {phase === "idle" && (
                        <motion.button
                          whileHover={{ borderColor: "rgba(59,130,246,0.3)", color: "#93c5fd" }}
                ...
                <motion.div
                  ...
                  transition={{ duration: 3, repeat: Infinity }}
                  className="flex items-center gap-2 mt-4 px-3 py-2.5 rounded-xl bg-purple-500/[0.07] border border-purple-500/15"
                >
                  <FiZap className="text-purple-400 text-xs shrink-0" />
                  <p className="text-gray-400 text-[11px]">
                    <span className="text-purple-300 font-semibold">DocuMind AI</span> will auto-summarize, extract key insights, and index each document.
                  </p>
                </motion.div>
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <motion.button
                    whileHover={files.length && phase === "idle" ? { scale: 1.03, boxShadow: "0 0 28px rgba(59,130,246,0.45)" } : {}}
                    whileTap={files.length && phase === "idle" ? { scale: 0.97 } : {}}
                    onClick={startUpload}
                    disabled={!files.length || phase === "uploading"}
                    className={`flex-2 flex-1 py-3 rounded-xl text-white text-sm font-semibold relative overflow-hidden flex items-center justify-center gap-2 transition-all duration-300 ${
                      !files.length ? "bg-white/10 text-gray-600 cursor-not-allowed"
                        : phase === "uploading" ? "bg-gradient-to-r from-blue-600/70 to-indigo-600/70 cursor-wait"
